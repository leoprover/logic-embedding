...
import leo.modules.embeddings.{Library, EmbeddingException, Embedding}
...
object EmbeddingApp {
  ...
  final def main(args: Array[String]): Unit = {
    ...
    if (args.isEmpty) usage()
      ...
      try {
        ...
        val maybeLogicSpec = getSpecFromProblem(parsedInput.formulas)
        ...
        val formulasToBeEmbedded = if (maybeLogicSpec.isDefined) parsedInput.formulas else {
  ...
  private[this] final def generateResult(problem: Problem, logicSpec: Option[AnnotatedFormula], embedding: Embedding): String = {
    ...
    if (logicSpec.isDefined) {
      ...
      sb.append(s"%%% ${logicSpec.get.pretty}\n")
    }
  ...
  private[this] final def getSpecFromProblem(formulas: Seq[TPTP.AnnotatedFormula]): Option[TPTP.AnnotatedFormula] = {
